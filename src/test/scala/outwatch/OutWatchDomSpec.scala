...
//    )
//
//    val SeparatedModifiers(streams, hasStreams, properties, emitters, Children.VNodes(nodes)) =
...
//    )
//
//    val SeparatedModifiers(_, hasStreams, properties, emitters, Children.StringModifiers(stringMods)) =
//      SeparatedModifiers.from(modifiers)
//
//    emitters.emitters.length shouldBe 3
//    properties.attributes.attrs.length shouldBe 1
...
//    val Children.VNodes(nodes) = modifiers.children
//
//    nodes.length shouldBe 3
//    modifiers.hasStreams shouldBe true
//
//    val proxy = modifiers.toSnabbdom("div").unsafeRunSync()
...
//
//    JSON.stringify(vtree.flatMap(_.toSnabbdom).unsafeRunSync()) shouldBe JSON.stringify(proxy)
//  }
//
...
//    val vtree = div(IO.pure(attributes.head), IO.pure(attributes(1)), child)
//
//    JSON.stringify(vtree.flatMap(_.toSnabbdom).unsafeRunSync()) shouldBe JSON.stringify(fixture.proxy)
//  }
//
//
...
//
//    JSON.stringify(vtree.flatMap(_.toSnabbdom).unsafeRunSync()) shouldBe JSON.stringify(expected)
//
//  }
//
